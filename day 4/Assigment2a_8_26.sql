...
select * from Employee where max(salary) and (trim(Last_name) <> 'o') ;

select * from Employee where (trim(Last_name) not like '%o');
select Last_name, max(salary) from Employee group by Last_name not like '%o%' ;
select Last_name, max(salary) from Employee group by Last_name having Last_name not like '%o%' ;