...
create function next_max_sal(m int)
returns int reads sql data
return (select max(salary) from Employee where salary < m);

select next_max_sal(4000000);
select distinct First_name from Employee where trim(First_name) like'[A,E,I,O,U]%';
select distinct First_name from Employee where trim(First_name) regexp '^[a,e,i,o,u]';
select distinct First_name from Employee;
drop function next_max;
delimiter &&
create function next_max(m int)
returns int reads sql data
begin
return (select max(salary) from Employee where salary < m);
end &&
...
-- Assignment11
-- 1. Create a table as same as employee table with true condition (but no columns used)
create table Employee_copy as select * from Employee where 5=5;
select * from Employee_copy;

-- 2. Create a table as same as employee table with true condition (but columns used)
create table Employee_copy_a as select * from Employee where Employee_id = '10';
select * from Employee_copy_a;

-- 3. Create a table as same as employee table with false condition (but columns used)
create table Employee_copy_b as select * from Employee where Employee_id = '15';
select * from Employee_copy_b;

-- 4. Create a table as same as employee table with false condition (but no columns used)
create table Employee_copy_c as select * from Employee where 24=27;
select * from Employee_copy_c;
select count(*) from Employee_copy_c;

-- 5. Write a trigger to convert negative salary to 0 before any insertion into employee Table
drop trigger chg_salato0;

insert into Employee values (12, ' Akila ', 'Ram', -3000, "2020-08-20", "Banking");
delimiter &&
create trigger chg_salato0
...
insert into Employee values (11, ' Poorna ', 'Ram', -3000, "2020-08-20", "Banking");
select * from Employee;

-- 6. Write a trigger to insert an entry into new table after any insertion into employee Table

CREATE TABLE Employee_mirror (
    Employee_id int ,
    First_name VARCHAR(50),
	Last_name VARCHAR(50),
	Salary int,
...
select * from Employee_mirror;

delimiter &&
create trigger Emp_mir_crt
after insert on Employee for each row
begin
	insert into Employee_mirror values (new.Employee_id, new.First_name, new.Last_name, new.Salary, new.Joining_date, new.Departement);
end &&
delimiter ;

INSERT INTO Employee (Employee_id, First_name, Last_name, Salary, Joining_date, Departement) VALUES (13, ' Adam ', 'Rose', 5500000, "2020-10-30", "hub");
select * from Employee;
select * from Employee_mirror;
...
delimiter &&
create trigger delafemp
after delete on Employee for each row
begin
	delete from Employee_mirror where Employee_id = old.Employee_id;
end &&
delimiter ;
...
-- Assignment 12
-- 1. Get department's total salary belonging to each employee's department
select * from Employee e,
(select Departement, sum(Salary) as Salary_sum from Employee group by Departement) f
where e.Departement = f.Departement;

-- 2. Get department's total salary belonging to each employee's department (including nulls)

INSERT INTO Employee (Employee_id, First_name, Last_name, Joining_date, Departement) VALUES (13, ' Aariz ', 'Badhu', "2019-10-20", "hub");
INSERT INTO Employee (Employee_id, First_name, Last_name, salary, Joining_date) VALUES (14, ' Azfer ', 'Badhu', 1002400 , "2019-10-20" );
select e.Employee_id, e.Departement, ifnull(e.Departement,'unk') as 'new_type', f.Salary_sum from Employee e,
(select Departement, sum(Salary) as Salary_sum from Employee group by Departement) f
where ifnull(e.Departement,'unk') = ifnull(f.Departement,'unk');
...
max(case when Departement = 'hub' then Salary else null end) as "hub",
...
select Employee_id, Departement, Salary, sum(Salary) over (order by Employee_id) as running_total from Employee;

-- 5. Get department's total salary belonging to each employee's department using window functions)
select Employee_id, Departement, Salary, sum(Salary) over (partition by Departement) as total_sum from Employee order by Employee_id;
...
select * from 
(select Employee_id, Departement, Salary, rank() over (order by Salary desc) as "rnk" from Employee)  e 
where e.rnk = 4;

select * from 
(select Employee_id, Departement, Salary, dense_rank() over (order by Salary desc) as "rnk" from Employee)  e 
where e.rnk = 4;

select Employee_id, Departement, Salary, rank() over (order by Salary desc) as "rnk",
dense_rank() over (order by Salary desc) as "dense_rnk" 
 from Employee;
 
-- Practice Queries 1
-- 1. Find Second Highest Salary from Employee table using subquery
select * from Employee e1 where 2 = (select count(distinct e2.salary) from Employee e2
where e1.salary <= e2.salary);
...
select * from (select max(salary) from Employee where salary not in (select max(Salary) from Employee)) as inline_sql;
select * from(select max(Salary) from Employee where Salary < (select max(Salary) from Employee)) as sec_high;


-- 4. Find Second Highest Salary from Employee table using rank window function
select * from 
(select Employee_id, Departement, Salary, rank() over (order by Salary desc) as "rnk" from Employee)  e where e.rnk = 2;
...
select Employee_id, Departement, Salary, rank() over(order by Salary desc) as rank_order from Employee;

-- 10. Rank each salary using dense rank function
select Employee_id, Departement, salary, dense_rank() over(order by Salary desc) as drank_or from Employee;

-- 11. Rank each salary "with out" using rank functions
select * from ( select Salary, row_number() over (order by salary desc) as row_num from Employee )e;

-- 12. Find Second Highest Salary from Employee table using "!=" any 
select max(salary) from Employee where Salary != (select max(Salary) from Employee);

-- 13. Find Second Highest Salary from Employee table using "left join and self join" anywhere in the query
Select e1.Employee_id, e1.Salary from Employee e1 inner join Employee e2 on e1.Salary = e2.Salary order by Salary desc limit 1,1;

-- 14. Find Second Highest Salary from Employee table using Row_number() function
select * from (select Salary, row_number() over (order by salary desc) as row_num from Employee)e where e.row_num = 2;
